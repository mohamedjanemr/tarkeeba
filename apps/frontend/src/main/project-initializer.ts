...
function ensureGitignoreEntries(projectPath: string, entries: string[]): void {
  ...
  if (fileExists) {
    ...
    writeFileSync(gitignorePath, '# Tarkeeba data directory\n' + entriesToAdd.join('\n') + '\n', 'utf-8');
  }
...
export function initializeProject(projectPath: string): InitializationResult {
  ...
  // Check git status - Tarkeeba requires git for worktree-based builds
  const gitStatus = checkGitStatus(projectPath);
  if (!gitStatus.isGitRepo || !gitStatus.hasCommits) {
    debug('Git check failed', { gitStatus });
    return {
      success: false,
      error: !gitStatus.isGitRepo
        ? 'Git repository required. Initialize Git and create an initial commit before setting up Tarkeeba.'
        : 'An initial Git commit is required before setting up Tarkeeba.'
    };
  }
 ...
 * not an installation. This allows Tarkeeba to be used to develop itself.
 */
