...
const detectAutoBuildSourcePath = (): string | null => {
  ...
  console.warn('[detectAutoBuildSourcePath] Could not auto-detect Tarkeeba source path. Please configure manually in settings.');
...
export function registerSettingsHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<AppSettings>> => {
      ...
      // See: https://github.com/AndyMik90/Aperant/issues/XXX
  ...
  ipcMain.handle(
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        // Generate content
        const lines: string[] = [
          '# Tarkeeba Framework Environment Variables',
